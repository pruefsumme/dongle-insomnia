...
prompt_iface() {
  ...
  while true; do
    ...
    if [[ -z "$choice" ]]; then
      echo "Please enter a choice."; continue
    fi
    ...
    # Interface name entered directly
...
prompt_method() {
  ...
  bold "Select keepalive method"
  echo "- auto: prefer local ARP keepalive when possible (recommended)"
  echo "- arp:  local-only ARP requests (requires arping + interface)"
  echo "- icmp: ICMP ping (works everywhere, may hit internet if TARGET is public)"
  echo
  echo "  [1] auto"
  echo "  [2] arp"
  echo "  [3] icmp"
